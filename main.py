...
from base.utils import ClipLoss, get_device, instantiate_from_config, update_config
...
class PLModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx == self.trainer.num_training_batches - 1:
            ...
            top_k_correct = (
                all_predicted_classes == all_true_labels[:, np.newaxis]
            ).any(axis=1)
    ...
    def on_validation_epoch_end(self):
        ...
        top_1_correct = top_1_predictions == all_true_labels
        top_1_accuracy = sum(top_1_correct) / len(top_1_correct)
        top_k_correct = (all_predicted_classes == all_true_labels[:, np.newaxis]).any(
            axis=1
        )
        top_k_accuracy = sum(top_k_correct) / len(top_k_correct)
        self.log(
    ...
    def on_test_epoch_end(self):
        ...
        self.match_similarities = (
            np.mean(self.match_similarities) if self.match_similarities else 0
        )
...
def main():
    ...
    train_loader, val_loader, test_loader = (
        load_eeg_data(config) if config["dataset"] == "eeg" else load_meg_data(config)
    )
    ...
    if config["exp_setting"] == "inter-subject":
        ...
        early_stop_callback = EarlyStopping(
            monitor="train_loss", min_delta=0.001, patience=5, verbose=False, mode="min"
        )
