...
fif_file = os.path.join(
    project_dir,
    f"ds004212-download/derivatives/preprocessed/preprocessed_P{sub}-epo.fif",
)

# output_dir = os.path.join(project_dir,"ds004212-download/derivatives/preprocessed_npy")

concept_csv_file_path = os.path.join(
    project_dir, "../things/THINGS/Metadata/Concept-specific/image_concept_index.csv"
)
csv_img_file_path = os.path.join(
    project_dir, "../things/THINGS/Metadata/Image-specific/image_paths.csv"
)
...
for i, event_id in enumerate(zs_event_ids):
    ...
    image_category_index = image_concept_df.iloc[
        event_id - 1, 0
    ]  # Accessing the first (and only) column at row i
...
train_set_categories_filtered = [
    item for item in train_set_categories if item not in test_set_categories
]

# Create a mask for epochs to keep in the training set
keep_epochs_mask = [
    category not in test_set_categories for category in train_set_categories
]
...
training_data = reshape_meg_data(
    training_epochs_filtered, num_concepts=1654, num_imgs=12, repetitions=1
)
print(training_data.shape)

zs_test_data = reshape_meg_data(
    zs_test_epochs, num_concepts=200, num_imgs=1, repetitions=12
)
...
img_path_list_training = [
    path.split("data/things-meg/Image_set/")[1] for path in img_path_list_training
]
img_path_list_test = [
    path.split("data/things-meg/Image_set/")[1] for path in img_path_list_test
]
