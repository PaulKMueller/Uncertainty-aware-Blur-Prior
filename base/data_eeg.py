...
from base.utils import get_device, instantiate_from_config
...
class EEGDataset(Dataset):
    def __init__(self, config, mode):
        ...
        if self.config["data"]["uncertainty_aware"]:
            ...
            for shift, tag in zip([-self.c, 0, self.c], ["low", "medium", "high"]):
                ...
                blur_param["params"]["blur_kernel_size"] = (
        ...
        ]  # transforms.Resize(pretrain_map[self.model_type]['resize']),
            ...
            if self.config["data"]["uncertainty_aware"]:
                ...
                for tag in ["low", "medium", "high"]:
                    self.img_features[tag] = self.ImageEncoder(
                        self.loaded_data[0]["img"], self.blur_transform[tag]
                    )
        ...
        for i in tqdm(range(0, len(set_images), batch_size)):
            ...
            ele = [
                self.process_transform(
                    blur_transform(
                        Image.open(
                            os.path.join(self.data_dir, "../Image_set_Resize", img)
                        ).convert("RGB")
                    )
                )
    ...
    def Textencoder(self, text):
        ...
        text_inputs = torch.cat(
            [open_clip.tokenize(f"This is a {t}.") for t in set_text]
        )
        device = next(self.vlmodel.parameters()).device
        ...
        text_features_dict = {
            set_text[i]: text_features[i].float().cpu() for i in range(len(set_text))
        }
